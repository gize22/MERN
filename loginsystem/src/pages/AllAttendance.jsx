...
const AllAttendance = () => {
  const { attendanceRecords, getAttendanceByDate, getAttendanceByDateRange } = useContext(AttendanceContext);
  const [records, setRecords] = useState([]);
  const [filterDate, setFilterDate] = useState('');
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [viewMode, setViewMode] = useState('all'); // 'all', 'date', 'range'
  ...
  useEffect(() => {
    loadRecords();
  }, [attendanceRecords, filterDate, startDate, endDate, viewMode]);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">All Attendance Records</h1>
